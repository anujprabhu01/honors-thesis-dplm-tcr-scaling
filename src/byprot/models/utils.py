...
class NetConfig:
    ...
    # SFT: load pretrained checkpoint and resize vocab
    sft_from_checkpoint: Optional[str] = None
...
def get_net(cfg):
    ...
    # SFT: load pretrained DPLM checkpoint and resize vocab for new tokens
    sft_ckpt_path = getattr(cfg.net, "sft_from_checkpoint", None)
    if sft_ckpt_path:
        from collections import OrderedDict

        # The net was created with the NEW vocab_size (e.g. 34) from config,
        # but the checkpoint has the OLD vocab_size (33). We must:
        # 1. Re-create net with old vocab_size to match checkpoint shapes
        # 2. Load checkpoint weights
        # 3. Resize embeddings to new vocab_size
        old_vocab = 33
        if cfg.net.arch_type == "esm":
            from byprot.models.dplm.modules.dplm_modeling_esm import EsmForDPLM
            from transformers import EsmConfig

            old_config = EsmConfig(
                hidden_size=cfg.net.hidden_size,
                num_hidden_layers=cfg.net.num_hidden_layers,
                num_attention_heads=cfg.net.num_attention_heads,
                intermediate_size=cfg.net.intermediate_size,
            ...
            old_config._name_or_path = "facebook/esm2_t6_8M_UR50D"
            net = EsmForDPLM(old_config, dropout=cfg.net.dropout)

        ckpt = torch.load(sft_ckpt_path, map_location="cpu")["state_dict"]
        new_state = {k[10:]: v for k, v in ckpt.items() if k.startswith("model.net.")}
        net.load_state_dict(new_state, strict=True)

        # Resize embeddings to accommodate new tokens (e.g., <sep>)
        net.resize_token_embeddings(cfg.net.vocab_size)
        # Fix LM head bias: copy pretrained bias, zero-init new tokens
        pretrained_bias = net.lm_head.bias
        net.lm_head.bias = nn.Parameter(torch.zeros(cfg.net.vocab_size))
        net.lm_head.bias.data[:old_vocab] = pretrained_bias.data[:old_vocab]
        del ckpt
